...
def get_IB_data(ID):
    ...
    # similarly for tst
    nb_classes = 2
    data_file = Path('../data/IB_data_'+str(ID)+'.npz')
    if data_file.is_file():
        data = np.load('../data/IB_data_'+str(ID)+'.npz')
    else:
        create_IB_data(ID)
        data = np.load('../data/IB_data_'+str(ID)+'.npz')
...
def create_IB_data(idx):
    ...
    np.savez_compressed('../data/IB_data_'+str(idx), X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test)
